...
for algo in ['primary-maxmin','primary-ratesum']:
    filename = f"03-15-2022_1000000_{algo}_0_86399_10_10_10.pkl"
    timestamps = []
    sum_values = []
    newList = []                                     
    ...
    plt.plot(np.sort(numpyL)[::-1], color=colors[algo], alpha=0.6, linewidth=0.7, label=labels[algo])
    # plt.plot(numpyL, color=colors[algo], alpha=0.6, linewidth=0.7, label=labels[algo])
plt.xlim([0,30])
plt.yscale('log')

pdf_filename = f"newPlots/fairness_ratesum_maxmin.pdf"
pdf_pages = PdfPages(pdf_filename)

plt.title("maxmin ratesum Fairness", fontsize=16)
